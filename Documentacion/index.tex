...
En este documento se va a documentar la dockerizacion de una API de gestion de usuarios y grupos y su subida a docker hub y su automatización con actions de github.
...
\begin{itemize}
    ...
    \item Workflow: Para las actions de github
...
\subsection{Hacer commit}
Ahora hacemos el commit para que funcione la subida de imagen, aquí vemos como ha funcionado
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{push.png}
\end{figure}

\section{Mensaje telegram}
Ahora vamos a añadirle que envie un mensaje por telegram cada vez que hagamos un se suba la imagen, para ello hay que crear un bot de telegram con el BotFather y cojer el token y el chatID y lo guardamos en secrets

\begin{figure}[H] Ahora le añadimos esto a nuestro yml
\begin{lstlisting}
- name: Notificar por Telegram
        if: success()  
        run: |
          MESSAGE="Nueva imagen subida a Docker Hub%0Repo: ${{ github.repository }}%0Commit: ${{ github.sha }}%0Imagen: ${{ secrets.DOCKER_IMAGE_NAME }}:latest"
...
Vemos como funciona aunque los saltos de línea no era con /n

\begin{figure}[H] 
    \centering
    \includegraphics[width=1\textwidth]{bot.png}
\end{figure}
